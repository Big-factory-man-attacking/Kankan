...
public:
    ...
    //注册
    //password:用户设置的密码
    Q_INVOKABLE void registerAccount(std::string password, std::string nickname);

    //登录
    //id:用户输入的帐号
    //key:用户输入的密码
    Q_INVOKABLE nlohmann::json login(std::string id, std::string password);

//    Q_INVOKABLE QJsonObject login();
